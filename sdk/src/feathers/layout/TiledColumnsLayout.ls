...
package feathers.layout
{
	...
	public class TiledColumnsLayout extends EventDispatcher implements IVirtualLayout
	{
		...
		public function getScrollPositionForIndex(index:int, items:Vector.<DisplayObject>, x:Number, y:Number, width:Number, height:Number):Point
		{
			...
			{
				HELPER_POINT.x = this._paddingLeft +  int((tileWidth + this._horizontalGap) * index / verticalTileCount) + (width - tileWidth) / 2;
				HELPER_POINT.y = 0;
			}
			return HELPER_POINT;
		}
		...
		protected function getVisibleIndicesAtScrollPositionWithHorizontalPaging(scrollX:Number, scrollY:Number, width:Number, height:Number, itemCount:int, result:Vector.<int>):void
		{
			...
			else if(partialPageSize > 0)
			{
				...
				columnOffset = Math.floor(partialPageSize / singleTileWidth);
		...
		protected function getVisibleIndicesAtScrollPositionWithoutPaging(scrollX:Number, scrollY:Number, width:Number, height:Number, itemCount:int, result:Vector.<int>):void
		{
			...
			if(totalColumnWidth < width)
			{
				if(this._verticalAlign == VERTICAL_ALIGN_BOTTOM)
				{
					columnIndexOffset = Math.ceil((width - totalColumnWidth) / singleTileWidth);
				}
				else if(this._verticalAlign == VERTICAL_ALIGN_MIDDLE)
				{
					columnIndexOffset = Math.ceil((width - totalColumnWidth) / singleTileWidth / 2);
				}
			}
			const columnIndex:int = -columnIndexOffset + Math.floor((scrollX - this._paddingLeft + this._horizontalGap) / singleTileWidth);
