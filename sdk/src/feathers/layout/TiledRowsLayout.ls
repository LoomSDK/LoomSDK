...
package feathers.layout
{
	...
	public class TiledRowsLayout extends EventDispatcher implements IVirtualLayout
	{
		...
		public function measureViewPort(itemCount:int, viewPortBounds:ViewPortBounds = null):Point
		{
			...
			HELPER_POINT.x = needsWidth ? Math.max(minWidth, totalWidth) : explicitWidth;
			HELPER_POINT.y = needsHeight ? Math.max(minHeight, totalHeight) : explicitHeight;
			return HELPER_POINT;
		}
		...
		public function getScrollPositionForIndex(index:int, items:Vector.<DisplayObject>, x:Number, y:Number, width:Number, height:Number):Point
		{
			...
			if(this._paging != PAGING_NONE)
			{
				...
				if(this._paging == PAGING_HORIZONTAL)
				{
					HELPER_POINT.x = pageIndex * width;
					HELPER_POINT.y = 0;
				}
				else
				{
					HELPER_POINT.x = 0;
					HELPER_POINT.y = pageIndex * height;
				}
			}
			else
			{
				HELPER_POINT.x = 0;
				HELPER_POINT.y = this._paddingTop + int((tileHeight + this._verticalGap) * index / horizontalTileCount) + (height - tileHeight) / 2;
			}
			return HELPER_POINT;
		}
		...
		protected function getVisibleIndicesAtScrollPositionWithHorizontalPaging(scrollX:Number, scrollY:Number, width:Number, height:Number, itemCount:int, result:Vector.<int>):void
		{
			...
			if(partialPageSize < 0)
			{
				...
				columnOffset = -Math.floor(partialPageSize / singleTileWidth) - 1;
			...
			else if(partialPageSize > 0)
			{
				...
				columnOffset = Math.floor(partialPageSize / singleTileHeight);
		...
		protected function getVisibleIndicesAtScrollPositionWithoutPaging(scrollX:Number, scrollY:Number, width:Number, height:Number, itemCount:int, result:Vector.<int>):void
		{
			...
			const singleTileHeight:int = Math.max(1, tileHeight + this._verticalGap);
			const horizontalTileCount:int = Math.max(1, int((width - this._paddingLeft - this._paddingRight + this._horizontalGap) / singleTileWidth));
			const verticalTileCount:int = Math.ceil((height - this._paddingTop + this._verticalGap) / singleTileHeight) + 1;
			...
			if(totalRowHeight < height)
			{
				if(this._verticalAlign == VERTICAL_ALIGN_BOTTOM)
				{
					rowIndexOffset = Math.ceil((height - totalRowHeight) / singleTileHeight);
				}
				else if(this._verticalAlign == VERTICAL_ALIGN_MIDDLE)
				{
					rowIndexOffset = Math.ceil((height - totalRowHeight) / singleTileHeight / 2);
				}
			}
			const rowIndex:int = -rowIndexOffset + Math.floor((scrollY - this._paddingTop + this._verticalGap) / singleTileHeight);
