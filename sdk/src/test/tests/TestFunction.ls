...
class FunctionMethodTestClass {
    ...
    public static function checkTestArgs(...args) {
        ...
        LegacyTest.assert(args != null, "checkTestArgs, null args");
                        
        LegacyTest.assert(args.length == argTest.length, "args.length != argTest.length");
            
        for (var argIdx:Number in args) {
            
            LegacyTest.assert(args[argIdx] == argTest[argIdx], "arg mismatch at index: " + argIdx);
...
class TestFunction extends LegacyTest
{
