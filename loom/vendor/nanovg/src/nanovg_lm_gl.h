...
static void glnvg__dumpShaderError(GLuint shader, const char* name, const char* type)
{
    char str[512 + 1];
    int len = 0;
    LGL->glGetShaderInfoLog(shader, 512, &len, str);
    if (len > 512) len = 512;
    str[len] = '\0';
...
static void glnvg__dumpProgramError(GLuint prog, const char* name)
{
    char str[512 + 1];
    int len = 0;
    LGL->glGetProgramInfoLog(prog, 512, &len, str);
    if (len > 512) len = 512;
    str[len] = '\0';
...
static int glnvg__renderCreateTexture(void* uptr, int type, int w, int h, int imageFlags, const unsigned char* data)
{
    ...
    if (imageFlags & NVG_IMAGE_GENERATE_MIPMAPS) {
        LGL->glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR_MIPMAP_LINEAR);
    }
    else {
        LGL->glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
    }
