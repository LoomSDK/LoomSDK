...
static seatest_int_string_varargs seatest_print = printf;
...
char* test_file_name(const char* inPath)
{
#ifdef _MSC_VER    
   char *path = (char*)_strdup(inPath);
#else
    char *path = (char*)strdup(inPath);
#endif
...
{
   char b[SEATEST_PRINT_BUFFER_SIZE];
   int l = (int)strlen(s);
   int d = (length - (l + 2)) / 2;
   int i;
   int len = 0;
   ...
   for(i = 0; i<d; i++) 
      len += snprintf(b + len, SEATEST_PRINT_BUFFER_SIZE - len, "%c", f);
   lmAssert(len < SEATEST_PRINT_BUFFER_SIZE, "Not enough space to print Seatest header (1)");
   
   if(l==0) 
      len += snprintf(b + len, SEATEST_PRINT_BUFFER_SIZE - len, "%c%c", f, f);
   else 
      len += snprintf(b + len, SEATEST_PRINT_BUFFER_SIZE - len, " %s ", s);
   lmAssert(len < SEATEST_PRINT_BUFFER_SIZE, "Not enough space to print Seatest header (2)");
   
   for(i = (d+l+2); i<length; i++) 
      len += snprintf(b + len, SEATEST_PRINT_BUFFER_SIZE - len, "%c",f);
   lmAssert(len < SEATEST_PRINT_BUFFER_SIZE, "Not enough space to print Seatest header (3)");
   
   len += snprintf(b + len, SEATEST_PRINT_BUFFER_SIZE - len, "\r\n");
   lmAssert(len < SEATEST_PRINT_BUFFER_SIZE, "Not enough space to print Seatest header (4)");
   seatest_print("%s", b);
}
...
{
   seatest_print("Usage: [-t <testname>] [-f <fixturename>] [-d] [help] [-v] [-m] [-k <marker>\r\n");
   seatest_print("Flags:\r\n");
   seatest_print("\thelp:\twill display this help\r\n");
   seatest_print("\t-t:\twill only run tests that match <testname>\r\n");
   seatest_print("\t-f:\twill only run fixtures that match <fixturename>\r\n");
   seatest_print("\t-d:\twill just display test names and fixtures without\r\n");
   seatest_print("\t-d:\trunning the test\r\n");
   seatest_print("\t-v:\twill print a more verbose version of the test run\r\n");
   seatest_print("\t-m:\twill print a machine readable format of the test run, ie :- \r\n");
   seatest_print("\t   \t<textfixture>,<testname>,<linenumber>,<testresult><EOL>\r\n");
   seatest_print("\t-k:\twill prepend <marker> before machine readable output \r\n");
   seatest_print("\t   \t<marker> cannot start with a '-'\r\n");
}

void seatest_set_print_callback(seatest_int_string_varargs printer)
{
   seatest_print = printer;
}
...
int seatest_parse_commandline_option_with_value(seatest_testrunner_t* runner, int arg, char* option, seatest_void_string setter)
{
   if(seatest_is_string_equal_i(runner->argv[arg], option))
   {
      ...
      {
         seatest_print("Error: The %s option expects to be followed by a value\r\n", option);
