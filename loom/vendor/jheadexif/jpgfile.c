...
// Parse the marker stream until SOS or EOI is seen;
//--------------------------------------------------------------------------
int ReadJpegSectionsFromBuffer (const char* data, int length, ReadMode_t ReadMode)
{
    int a;
    int HaveCom = FALSE;
    int offset = 0;
    ...
    for(;;){
        ...
        if (offset + itemlen-2 > length)
        {
            // Premature end of file?
            return FALSE;
        }

        memcpy(Data+2, &data[offset], itemlen-2); // Read the whole section.
        offset += itemlen-2;
...
//--------------------------------------------------------------------------
int ReadJpegBuffer(const char *data, int length, ReadMode_t ReadMode)
{
    int ret;

    // Scan the JPEG headers.
    ret = ReadJpegSectionsFromBuffer(data, length, ReadMode);
