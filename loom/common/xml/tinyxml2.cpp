...
int XMLDocument::LoadFile( const char* filename )
{
  ...
  if(!platform_mapFile(filename, &ptr, &size)) {
...
{
	lmAssert(false, "XMLDocument::LoadFile( FILE* fp ) is no longer supported. Please use XMLDocument::LoadFile( const char* filename )");
