...
template<typename T> UT_INLINE void utSwap(T& a, T& b)
{
    T t(a);
...
template<typename T> UT_INLINE T utMax(const T& a, const T& b)
{
    return a < b ? b : a;
}


template<typename T> UT_INLINE T utMin(const T& a, const T& b)
{
    return a < b ? a : b;
}


template<typename T> UT_INLINE T utClamp(const T& v, const T& a, const T& b)
{
...
public:
    ...
    // Note: Using an ordered erase is slow
    void erase(const T& v, bool ordered = false)
    {
        erase(find(v), ordered);
    }

    // Note: Using an ordered erase is slow
    void erase(UTsize pos, bool ordered = false)
    {
        if (m_size > 0)
        {
            assert(pos != UT_NPOS);             // NOTE: This is just here to help us ensure
                                                //  that we're always removing things when we think we are.
...
public:
    ...
    utHashTable()
        :    m_size(0), m_capacity(0), m_lastPos(UT_NPOS), m_lastKey(UT_NPOS),
