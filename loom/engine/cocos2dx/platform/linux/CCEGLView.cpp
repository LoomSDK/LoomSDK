...
// GLFW window close handler defined in CCApplication.cpp
int windowCloseEventHandle();
...
bool CCEGLView::Create(const char *pTitle, int iPixelWidth, int iPixelHeight, int iWidth, int iHeight, int iDepth)
{
    ...
    if (eResult)
    {
        ...
        // TODO: LOOM-954 - there is no way to specify position on open window call
        // once there is, we should do so in the create call in not center window here
        centerWindow();
        ...
        glfwSetWindowCloseCallback(windowCloseEventHandle);
...
void CCEGLView::centerWindow()
{
    ...
    int x = vidmode.Width / 2;
    int y = vidmode.Height / 2;

    x -= m_sSizeInPoint.width / 2;
    y -= m_sSizeInPoint.height / 2;
...
{
    Create(m_szViewName, width, height, width, height, 32);
