...
#include "loom/script/native/core/system/lmDebug.h"
...
bool Debug::filterMethodBase(MethodBase *methodBase)
{
...
int Debug::getCallStack(lua_State *L, EventType eventType)
{
...
void Debug::debugHook(lua_State *L, lua_Debug *ar)
{
    int top = lua_gettop(L);
...
int Debug::loomAssert(lua_State *L)
{
    bool fail = false;
...
int Debug::getLocals(lua_State *L)
{
...
int Debug::getBreakpoints(lua_State *L)
{
    Type *vectorType = LSLuaState::getLuaState(L)->getType("system.Vector");
...
bool Debug::hasBreakpoint(const char *source, int line)
{
...
void Debug::regenerateSourceBreakpoints()
{
...
void Debug::addBreakpoint(const char *source, int line)
{
    Breakpoint *bp;
...
void Debug::removeBreakpoint(const char *source, int line)
{
    for (UTsize i = 0; i < breakpoints.size(); i++)
    {
        Breakpoint *bp = breakpoints.at(i);
...
void Debug::removeAllBreakpoints()
{
    for (UTsize i = 0; i < breakpoints.size(); i++)
    {
...
void Debug::removeBreakpointAtIndex(int index)
{
