...
public delegate ExecutorFuture(f:Future):Void;
public delegate ExecutorSelf(e:Executor):Void;
...
public class Executor implements IAnimatable
{
    ...
    public function submitAll (fs :Vector.<Function>) :Vector.<Future> {
    ...
    /** @private */
    public function advanceTime(time:system.Number):Void {
        runIfAvailable();
        if (_toRun.length == 0) {
