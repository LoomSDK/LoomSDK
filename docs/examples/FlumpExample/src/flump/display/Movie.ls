...
public delegate LabelCallback(label:String):void;
    ...
    implements IAnimatable
{
    ...
    public const labelPassed:LabelCallback;
    ...
    */
    ...
    protected function updateFrame (newFrame :int, dt :Number) :void {
        ...
        for (var ii :int = 0; ii < frameCount; ++ii) {
            ...
            if (_labels[frameIdx] != null) {
                var frameLabels:Vector.<String> = _labels[frameIdx];
                for each (var label :String in frameLabels) {
                    labelPassed(label);
                    if (_pendingFrame != NO_FRAME) break;
                }
            }
    ...
    /** @private */
    public var _playerData :MoviePlayerNode;
    /** @private */
