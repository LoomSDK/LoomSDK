...
package org.robotlegs.base
{
	...
	public class ViewMap extends ViewMapBase implements IViewMap
	{
		 ...
		 * @private
		 */
		protected var mappedPackages:Vector.<Object>;

		/**
		 * @private
		 */
		protected var mappedTypes:Dictionary.<String, Type>;

		/**
		 * @private
		 */
		protected var injectedViews:Dictionary.<String, Object>;
		...
		public function ViewMap(contextView:DisplayObjectContainer, injector:IInjector)
		{
			...
			this.mappedPackages = new Vector.<Object>();
		 ...
		 * @inheritDoc
		 */
		public function mapType(type:Type):void
		{
			if (mappedTypes[type])
				return;
		 ...
		 * @inheritDoc
		 */
		public function unmapType(type:Type):void
		{
			var mapping:Type = mappedTypes[type];
			
			//delete mappedTypes[type];
			mappedTypes[type] = null;
		 ...
		 * @inheritDoc
		 */
		public function hasType(type:Type):Boolean
		{
		...
		protected override function onViewAdded(e:Event):void
		{
			...
			for each (var type:Type in mappedTypes)
			{
				if (target is type)
				{
					injectInto(target);
			...
			if (len > 0)
			{
				var className:String = target.getTypeName(); //getQualifiedClassName(target);
				for (var i:int = 0; i < len; i++)
				{
					var packageName:String = mappedPackages[i] as String;
