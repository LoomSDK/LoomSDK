package loom.modestmaps.overlays
{
    ...
    import loom.modestmaps.events.MarkerClick;
    ...
    public class MarkerClip extends Sprite
    {
        ...
        public function MarkerClip(map:Map)
        {
            ...
            //map.addEventListener(MapEvent.PANNED, onMapPannedEvent);
            //map.addEventListener(MapEvent.RESIZED, onMapResizedEvent);
            //map.addEventListener(MapEvent.EXTENT_CHANGED, onMapExtentChangedEvent);
            //map.addEventListener(MapEvent.RENDERED, updateClipsEvent);
            //map.addEventListener(MapEvent.MAP_PROVIDER_CHANGED, onMapProviderChangedEvent);
            map.onZoom += onMapZoom;
            map.onPan += onMapPanned;
            map.onResize += onMapResized;
            map.onExtentChange += onMapExtentChanged;
            map.onMapRender += updateClips;
            map.onProviderChange += onMapProviderChanged;
        ...
        public function updateClipsEvent(event:Event = null) { updateClips(); }
        ...
        protected function onMapExtentChangedEvent(event:MapEvent):void { dirty = true; }
        protected function onMapExtentChanged(state:MapState, extent:MapExtent):void
        {
            if (state == MapState.STOPPED) dirty = true;           
        }
        
        protected function onMapPannedEvent(event:MapEvent):void
        {
            if (drawCoord) {
        ...
        protected function onMapZoom(state:MapState, level:Number, delta:Number):void
        {
        ...
        protected function onMapResizedEvent(event:MapEvent):void { onMapResized(0, 0); }
        ...
        protected function onMapProviderChangedEvent(event:MapEvent):void { onMapProviderChanged(null); }
