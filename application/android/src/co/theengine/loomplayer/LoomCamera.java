...
public class LoomCamera
{
    ...
    public static void triggerCameraIntent(Activity ctx)
    {
        ...
        if(!LoomPlayer.checkPermission(ctx, "android.permission.CAMERA"))
        {
    ...
    public static void onActivityResult(Activity ctx, int requestCode, int resultCode, Intent data) 
    {
        ...
        if (resultCode == Activity.RESULT_OK) 
        {
            ...
            // Fire results event.
            LoomPlayer.triggerGenericEvent("cameraSuccess", fileUri.getEncodedPath());
        } 
        else if (resultCode == Activity.RESULT_CANCELED) 
        {
            // User cancelled the image capture
            LoomPlayer.triggerGenericEvent("cameraFail", "cancel");
        }
        else 
        {
            // Image capture failed, advise user
            LoomPlayer.triggerGenericEvent("cameraFail", "fail");
        }
    }    
}